...
int InjectRemoteProcess(int nPid, const char *lpLibraryPath, const char *lpFunctionName, const char *lpFunctionParam)
{
    ...
    {
        DEBUG_PRINT("[-]pfndlopen | pfndlsym | pfndlclose == NULL\r\n");
        nRet = -1;
        goto SAFE_END;
    }
...
#define FUNCTION_PARAM_OFFSET   0x200
    if (PtraceWriteProcessMemory(nPid, lpMmapBase + FUNCTION_PARAM_OFFSET, lpFunctionParam, strlen(lpFunctionParam) + 1) == -1)
    {
        DEBUG_PRINT("[-]InjectRemoteProcess::PtraceWriteProcessMemory Error\r\n");
        nRet = -1;
        goto SAFE_END;
    ...
    ParamArg[0] = lpMmapBase + FUNCTION_PARAM_OFFSET;

    if (CallRemoteFunction(nPid, "entry_hook", pfnRemoteFunction, ParamArg, 1, &Regs) == -1)
    {
        nRet = -1;
        goto SAFE_END;
    }
...
{
    int nProId = FindProIdByProName("/system/bin/surfaceflinger");
    if (nProId == -1)
    {
        return -1;
    }

    if (InjectRemoteProcess(nProId, "/data/libmixi.so", "entry_hook", "Hello Inject") == -1)
    {
