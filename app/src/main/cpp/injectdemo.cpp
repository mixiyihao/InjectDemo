...
int FindProIdByProName(const char *lpszProName)
{
    ...
    if(lpDirp == NULL)
    {
        printf("[-]FindProIdByProName::opendir error\r\n");
        return -1;
    }
...
void* GetModuleBase(int nPid, const char *lpLibraryPath)
{
    ...
    if (fp != NULL)
    {
        while (fgets(szLines, sizeof(szLines), fp))
        {
            ...
            {
                lpCh = strtok(szLines, "-");
                lpBaseAddr = (void *)strtoul(lpCh, NULL, 16);
                fclose(fp);
                break;
            }
        }
...
void* GetRemoteFunctionAddr(int nPid, const char *lpLibraryPath, void *lpFunctionAddr)
{
    ...
    lpRemoteFunctionAddr = (void *)((uintptr_t)lpRemoteBaseAddr - (uintptr_t)lpLocalBaseAddr + (uintptr_t)lpFunctionAddr);
...
int PtraceWriteProcessMemory(int nPid, void *lpAddr, const uint8_t *lpData, uint32_t nLength)
{
    ...
    j = nLength / 4;
    remain = nLength % 4;
    memcpy(lpDataBuff,lpData,strlen((const char *)lpData));
    //先4字节拷贝
    for (i = 0; i < j; i++)
    {
...
int PtraceCallRemoteFunction(int nPid, void *pfnFunctionAddr, long *lpParamArg, int nParamCount, struct pt_regs *lpRegs)
{
    ...
    //将PC的值设置为函数地址
    lpRegs->ARM_pc =(long) pfnFunctionAddr;
...
int InjectRemoteProcess(int nPid, const char *lpLibraryPath, const char *lpFunctionName, const char *lpFunctionParam)
{
    ...
    extern struct pt_regs Regs = {0};
    extern struct pt_regs OldRegs = {0};
    long ParamArg[10] = {0};
    void *hSo = NULL;
    ...
    if (PtraceWriteProcessMemory(nPid, lpMmapBase, (uint8_t *)lpLibraryPath, strlen(lpLibraryPath) + 1) == -1)
    {
        nRet = -1;
        goto SAFE_END;
    }
...
#define FUNCTION_NAME_OFFSET    0x100
    ...
    if (PtraceWriteProcessMemory(nPid, lpMmapBase + FUNCTION_NAME_OFFSET, (uint8_t *)lpFunctionName, strlen(lpFunctionName) + 1) == -1)
    {
        nRet = -1;
        goto SAFE_END;
    }

    ParamArg[1] = (long)(lpMmapBase + FUNCTION_NAME_OFFSET);
    ...
    pfnRemoteFunction = (void *)Regs.ARM_r0;

    //传递参数, 准备调用注入模块中的函数  void MyTest(const char *lpstr)
#define FUNCTION_PARAM_OFFSET   0x200
    if (PtraceWriteProcessMemory(nPid, lpMmapBase + FUNCTION_PARAM_OFFSET, (uint8_t *)lpFunctionParam, strlen(lpFunctionParam) + 1) == -1)
    {
        nRet = -1;
        goto SAFE_END;
    }

    ParamArg[0] = (long)lpMmapBase + FUNCTION_PARAM_OFFSET;

    if (CallRemoteFunction(nPid, "MyTest", pfnRemoteFunction, ParamArg, 1, &Regs) == -1)
    {
        nRet = -1;
        goto SAFE_END;
    }
...
int main(int argc, char* argv[])
{
    ...
    if (InjectRemoteProcess(nProId, "/data/local/tmp/MyTest.so", "MyTest", "Hello Inject") == -1)
    {
